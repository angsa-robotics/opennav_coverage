  ...
  const std_msgs::msg::Header & header, const bool is_cartesian)
{
  ...
  PathSectionType curr_state = path.getStates()[0].type;
  if (curr_state == PathSectionType::SWATH) {
    curr_swath_start = path.getStates()[0].point;
  ...
  for (unsigned int i = 0; i != path.getStates().size(); i++) {
    if (curr_state == path.getStates()[i].type && path.getStates()[i].type == PathSectionType::SWATH) {
      // Continuing swath so...
      // (1) no action required.
    } else if (curr_state == path.getStates()[i].type && path.getStates()[i].type == PathSectionType::TURN) {
      // Continuing a turn so...
      // (1) keep populating
      curr_turn->poses.push_back(toMsg(path.getStates()[i]));
    } else if (curr_state != path.getStates()[i].type && path.getStates()[i].type == PathSectionType::TURN) {
      ...
      swath.end = toMsg(path.getStates()[i - 1].point);
      ...
      curr_turn = &msg.turns.back();
      curr_turn->poses.push_back(toMsg(path.getStates()[i]));
    } else if (curr_state != path.getStates()[i].type && path.getStates()[i].type == PathSectionType::SWATH) {
      // Transitioning from a turn to a swath so...
      // (1) Update new swath starting point
      curr_swath_start = path.getStates()[i].point;
    }

    curr_state = path.getStates()[i].type;

    if (path.getStates()[i].type != PathSectionType::SWATH &&
      path.getStates()[i].type != PathSectionType::TURN)
    {
  ...
  const float & pt_dist)
{
  ...
  for (unsigned int i = 0; i != path.getStates().size(); i++) {
    ...
    if (i > 0 && path.getStates()[i].type == PathSectionType::SWATH &&
      path.getStates()[i - 1].type == PathSectionType::SWATH)
    {
      const float & x0 = path.getStates()[i - 1].point.getX();
      const float & y0 = path.getStates()[i - 1].point.getY();
      const float & x1 = path.getStates()[i].point.getX();
      const float & y1 = path.getStates()[i].point.getY();
      ...
      pose.pose.orientation =
        nav2_util::geometry_utils::orientationAroundZAxis(path.getStates()[i].angle);
      pose.pose.position.x = x0;
      pose.pose.position.y = y0;
      pose.pose.position.z = path.getStates()[i].point.getZ();
      ...
      msg.poses.push_back(toMsg(path.getStates()[i]));
    }
  }
